...
<div title="人员信息" style="padding:10px;width:98%;margin-top:20px;">
    ...
    <br/><br/>
    <div><font style="color:red;font-size:14px;font-family:微软雅黑">备注：人员统计以【学历】不为空且数据合法为准。</font></div>
    <br/>
    <div id="dg_rsxx"></div>
...
<script type="text/javascript">
    ...
    function cx() {
        ...
        var content = '<chart caption="' + Tgslb + Tryzt + '员工学历结构饼状图" baseFontSize="16" showNames="1" showBorder="0" baseFont="微软雅黑"  outCnvBaseFontSize="100" bgColor="EEF3FA" canvasBgColor="EEF3FA" formatNumberScale="0" canvasBorderThickness="0"  showValues="1" showYAxisValues="0" showLegend="0"  labelDisplay="STAGGER" showPlotBorder="1" numDivLines="0"  borderThickness ="0" yAxisName="" xAxisName="" numberSuffix="人" plotSpacePercent="50">';
        var myChart = null;

        var url = "servlet/U8Action?method=rsxl&gslb=" + gslb + "&kssj=" + kssj + "&jssj=" + jssj + "&ryzt=" + ryzt;
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: url,
            ...
            success: function (data) {
                ...
                if (data.length > 0) {
                    ...
                    var lb6 = data[0]['lb6'];
                    var zs = data[0]['zs'];
                    var nljg = "<table id='rsgl'><tr><td  colspan=4 class='zbt'>" + Tgslb + Tryzt + "员工学历结构表格</td></tr>" +
                        "<tr><td  colspan=4 class='rs'>" + dzrq + Tryzt + "员工总数：" + zs + "人</td></tr>" +
                        "<tr><td  class='xh'>序号</td><td  class='bt'>学历范围</td><td  class='bt'>总人数</td><td  class='bt'>百分比</td></tr>" +
                        "<tr><td class='nr'>1</td><td class='nr'>初中及以下</td><td class='nr'>" + lb1 + "</td><td class='nr'>" + (lb1 / zs * 100).toFixed(2) + "%</td></tr>" +
                        "<tr><td class='nr'>2</td><td class='nr'>中专/高中</td><td class='nr'>" + lb2 + "</td><td class='nr'>" + (lb2 / zs * 100).toFixed(2) + "%</td></tr>" +
                        "<tr><td class='nr'>3</td><td class='nr'>大专/高职</td><td class='nr'>" + lb3 + "</td><td class='nr'>" + (lb3 / zs * 100).toFixed(2) + "%</td></tr>" +
                        "<tr><td class='nr'>4</td><td class='nr'>本科</td><td class='nr'>" + lb4 + "</td><td class='nr'>" + (lb4 / zs * 100).toFixed(2) + "%</td></tr>" +
                        "<tr><td class='nr'>5</td><td class='nr'>研究生及以上</td><td class='nr'>" + lb5 + "</td><td class='nr'>" + (lb5 / zs * 100).toFixed(2) + "%</td></tr>" +
                        "</table>";
                    $("#dg_rsxx").html(nljg);

                    content += '<set label="初中及以下"  value="' + lb1 + '" /><set label="中专/高中"  value="' + lb2 + '" /><set label="大专/高职"  value="' + lb3 + '" /><set label="本科"  value="' + lb4 + '" /><set label="研究生及以上"  value="' + lb5 + '" /></chart>';
