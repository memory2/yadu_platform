...
<title>人员信息管理</title> 
  ...
  %>
	...
	<script type="text/javascript">
	        ...
	        $("#bm").combobox({
	        	  onChange: function (n,o) {
		    	        if(txm=="10297"){
		    	        	...
		    	        	if($("#bm").combobox('getValue').substring(0,4)!="4106"&&$("#bm").combobox('getValue').substring(0,6)!="410510"&&$("#bm").combobox('getValue')!="4100000000"){
		    	        		alert("您无此部门查询权限！");
				  ...
				  $.ajax({  
		              ...
		              success: function(data){
		            		  ...
		            		  //$("#dg_kqxx").html(JSON.stringify(data));
		            		   ...
		            		   var kqtj = "<table id='tjkq'><tr><td style='font-size:14px;font-weight:bold' colspan=18>亚都实业"+kqy+"考勤汇总表</td></tr><tr><td  class='xh' rowspan=2>序号</td><td class='nr_bt' rowspan=2>部门</td><td class='nr_bt' rowspan=2>工号</td><td class='nr_bt' rowspan=2>姓名</td><td class='nr_bt' colspan=2>出勤天数</td><td class='nr_bt' colspan=6>休假天数</td><td class='nr_bt' rowspan=2>加班小时数</td><td class='nr_bt' colspan=5>迟到、早退、补签次数</td></tr><tr><td class='nr_bt'>应出勤</td><td class='nr_bt'>实际出勤</td><td class='nr_bt'>公司放假</td><td class='nr_bt'>可调休天数</td><td class='nr_bt'>事假</td><td class='nr_bt'>婚假</td><td class='nr_bt'>丧假</td><td class='nr_bt'>工伤</td><td class='nr_bt'>迟到15分钟以内</td><td class='nr_bt'>迟到15分钟以上</td><td class='nr_bt'>早退</td><td class='nr_bt'>补签</td><td class='nr_bt'>合计</td></tr>";
 	 ...
 	 <style>
     tr td{border:solid #add9c0; border-width:1px 1px 1px 1px; padding:3px 0px;text-align:center;font-size:12px;white-space:0;width:50px;}
     .bt_td{height:35px;}
     .wqd{height:35px;color:#CCD0FF}
     .zc{height:35px;color:#008000}
     .gx{height:35px;color:#800080}
     .cc{height:35px;color:#0000FF}
     .cdzt{height:35px;color:#FF4500}
     .bq{height:35px;color:#0000FF}
     .qj{heght:35px;color:#FF0000}
     .qt{heght:35px;color:#FF8C00}
     .mq{height:35px;color:#9932CC}
     
	 #nf,#yf{width:70px}
	 #dg_kqxx{width:1900px!important; border-collapse:collapse; padding-right:20px;}
	 
	 #tj_kqxx{width:100%!important;border-collapse:collapse; padding-right:15px;overflow:auto}
	 .rycs{text-align:left;padding-left:5px;font-weight:bold;}
	 .bt{width:70px;}
	 .nr{width:60px;}
	 .nr_bt{width:79px;}
	 .xh{width:40px;}
	 #kqy_tj{font-size:14px;font-weight:bold;padding:5px;}
	 .progress{z-index: 2000} 
     .mask{position: fixed;top: 0;right: 0;bottom: 0;left: 0; z-index: 1000; background-color: #2F2F2F} 
	 #fhdb{font-size:12px;text-align:right;padding-right:15px;display:none}
	 .bzsm{text-align:left;padding-left:5px;color:red}
	 </style>
</head> 
		    	  ...
		    	  <form id="jbxxForm" method="post" >
		    	     <table style="width:100%;border-collapse:collapse;text-align:center;font-size:12px;" id="cxtj">
		    	     	<tr>
		    	     	  ...
		    	     	  <td  style="width:180px;"> <input id="yf" class="easyui-combobox" value="" name='yf' data-options=" panelHeight:'auto', valueField:'id',textField:'text',data: [{id: '1',text: '本月'},{id: '2',text: '上月'}]" /></td>
		    	     	   <td style="width:100px;text-align:center">所属部门</td>
		    	     	  <td  style="width:180px;"><input id="bm" class="easyui-combotree" data-options="valueField:'id',panelHeight:'auto',textField:'text'"  name="ssbm" style="width:150px;"/>  
		    	     	  </td>
		    	     	  <td style="width:100px;">姓名</td>
		    	     	  <td style="width:180px;"><input id="xm" class="easyui-combobox" name="xm" style="width:150px;"  data-options="panelHeight:'260'" /></td>
		    	     	    <td style="width:300px;">
		    	     	       ...
		    	     	       <a id="btn_export_kqxx" class="easyui-linkbutton" data-options="iconCls:'icon-dload'" style="display:none">导出统计</a>
		    	     	    </td>
		    	     	<tr>
		    	     </table>
		    	  </form>
